...
> - Olympus Vault borrows delegated `USDC` from **Aave** using **credit delegation**.
...
In simple terms: your deposit goes into Olympus Vault, the vault borrows extra USDC from Aave (through delegated credit), then both are used in Uniswap v4 to run the strategy.
...
Olympus Vault draws USDC from **Aave** through a **credit delegation** line assigned to the vault.
...
That is the key intuition: reducing `sqrt(p)` drag is how Olympus targets IL at the root strategy level, instead of relying only on token incentives.
...
| Asset move | Just hold (1 ETH + $2,000 USDC) | Classic `x*y=k` LP | Olympus (2x leverage) |
| --- | --- | --- | --- |
...
## 🛡️Protection Layer: Why Olympus Is Different
